...
3. Skill `allowed-tools` 真正落地：让 Skill 从“上下文注入”升级为“运行时可约束的平台能力”。
4. 命令执行后端抽象：把 `run_command` 的工具协议和本地进程执行边界彻底拆开，补齐 Runtime 工程完整度。
5. 演示与项目说明：把已有能力沉淀成 README、demo 脚本与可直接支撑简历表述的证据。
...
11. skill 渐进式加载
12. 命令执行后端抽象与本地 sandbox 语义固定
13. background task
14. 最小评估链路
15. JPA + PostgreSQL 持久化
16. SSE 推送
17. 显式模型路由、Handoff 与 ACP adapter 边界预留
